...
class RoleCommand extends Command {
  constructor() {
    super('role', {
      aliases: ['role', 'roleinfo', 'role-info', 'ri'],
  ...
  exec(message, args) {
    ...
    const permissions = Object.keys(PERMISSIONS).filter(
		...
		const permsList = permissions.includes('ADMINISTRATOR') ? ['ADMINISTRATOR'] : permissions;

    const embed = new MessageEmbed()
      .setColor(role.color)
      .setDescription(`Info about **${role.name}** (ID: ${role.id})`)
      .setThumbnail(message.guild.iconURL())
      .addField('❯ Info', `• Color: ${role.hexColor.toUpperCase()}\n• Hoisted: ${role.hoist ? 'Yes': 'No'}\n• Mentionable: ${role.mentionable ? 'Yes' : 'No'}\n• Creation Date: ${moment(role.createdAt).format('dddd, MMMM Do YYYY, h:mm:ss A')}`)
      .addField('❯ Permissions', `${permsList.map((permission) => `• ${PERMISSIONS[permission]}`).join('\n') || 'None'}`);
